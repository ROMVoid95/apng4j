...
import net.romvoid95.apng4j.error.PngFeatureException;
 ...
 * each able to transform the input into ARGB output pixels.
 */
public class ArgbProcessors {

    /**
     * Determine from the header the concrete ArgbProcessor implementation the
     * caller needs to transform the incoming pixels into the ARGB pixel format.
     *
     ...
     * @return a concrete ArgbProcessor to transform source pixels into the specific bitmap.
     * @throws PngException if there is a feature not supported or some specification break with the file.
     */
    public static ScanlineProcessor from(PngHeader header, PngScanlineBuffer scanlineReader, ArgbBitmap bitmap) throws PngException {
        ...
        switch (header.colourType) {
            case PNG_GREYSCALE:
                switch (header.bitDepth) {
                    case 1:
                        return new IndexedColourBits(bytesPerScanline, bitmap, 7, 0x01, PngConstants.SHIFTS_1, ArgbPalette.forGreyscale(1));
                    case 2:
                        return new IndexedColourBits(bytesPerScanline, bitmap, 3, 0x03, PngConstants.SHIFTS_2, ArgbPalette.forGreyscale(2));
                    case 4:
                        return new IndexedColourBits(bytesPerScanline, bitmap, 1, 0x0F, PngConstants.SHIFTS_4, ArgbPalette.forGreyscale(4));
                    case 8:
            ...
            default:
                throw new PngFeatureException("ARGB doesn't support PNG mode "+header.colourType.name());
        }
    }

    /**
     * Transforms 1-, 2-, 4-bit indexed colour source pixels to ARGB pixels.
     */
    public static class IndexedColourBits extends ScanlineProcessor {

        private int highBit;
        private int mask;
        private byte[] shifts;

        public IndexedColourBits(int bytesPerScanline, ArgbBitmap bitmap, int highBit, int mask, byte[] shifts) {
            this(bytesPerScanline, bitmap, highBit, mask, shifts, null);
        }

        public IndexedColourBits(int bytesPerScanline, ArgbBitmap bitmap, int highBit, int mask, byte[] shifts, ArgbPalette palette) {
        ...
        @Override
        public ScanlineProcessor clone(int bytesPerRow, ArgbBitmap bitmap) {
            return new IndexedColourBits(bytesPerRow, bitmap, highBit, mask, shifts, palette);
        }
    }

    /**
     * Special case implementation to transform 8-bit indexed colour source pixels to ARGB pixels.
     */
    public static class IndexedColour8 extends ScanlineProcessor {

        public IndexedColour8(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms 4-bit greyscale with alpha source pixels to ARGB pixels.
     */
    public static class Greyscale4Alpha extends ScanlineProcessor {

        public Greyscale4Alpha(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms 8-bit greyscale source pixels to ARGB pixels.
     */
    public static class Greyscale8 extends ScanlineProcessor {

        boolean haveTransparent = false;
        int transparentSample = 0;

        public Greyscale8(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms 8-bit greyscale with alpha source pixels to ARGB pixels.
     */
    public static class Greyscale8Alpha extends ScanlineProcessor {

        public Greyscale8Alpha(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms 16-bit greyscale with alpha source pixels to ARGB pixels.
     */
    public static class Greyscale16Alpha extends ScanlineProcessor {

        public Greyscale16Alpha(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms true-colour (RGB) 8-bit source pixels to ARGB pixels.
     */
    public static class Truecolour8 extends ScanlineProcessor {
        public Truecolour8(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms true-colour with alpha (RGBA) 8-bit source pixels to ARGB pixels.
     */
    public static class Truecolour8Alpha extends ScanlineProcessor {

        public Truecolour8Alpha(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms true-colour (RGB) 16-bit source pixels to ARGB pixels.
     */
    public static class Truecolour16 extends ScanlineProcessor {
        public Truecolour16(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
    ...
    /**
     * Transforms true-colour with alpha (RGBA) 16-bit source pixels to ARGB pixels.
     *
     * Note that the simpler method of resampling the colour is done, namely discard the LSB.
     */
    public static class Truecolour16Alpha extends ScanlineProcessor {
        public Truecolour16Alpha(int bytesPerScanline, ArgbBitmap bitmap) {
            super(bytesPerScanline, bitmap);
        }
