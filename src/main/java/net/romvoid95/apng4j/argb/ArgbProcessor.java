...
import net.romvoid95.apng4j.PngChunkCode;
...
import net.romvoid95.apng4j.reader.PngChunkProcessor;

/**
 * Concrete implementation of a chunk processor designed to process pixels into
 * 32-bit ARGB format.
 */
public class ArgbProcessor<ResultT> implements PngChunkProcessor<ResultT> {

	protected PngHeader header = null;
	protected PngScanlineBuffer scanlineReader = null;
	protected ArgbDirector<ResultT> builder = null;
	protected ScanlineProcessor scanlineProcessor = null;


	public ArgbProcessor(ArgbDirector<ResultT> builder) {
	...
	public void processPalette(byte[] bytes, int position, int length) throws PngException {
		builder.receivePalette(ArgbPalette.fromPaletteBytes(bytes, position, length));
	}
